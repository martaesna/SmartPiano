...
public class PianoController implements ActionListener, MouseListener, KeyListener {
    ...
    public PianoController(PianoView pianoView){
        this.pianoView = pianoView;
        this.pianoView.addKeyListener(this);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        switch (e.getActionCommand()){
            case "libre":
                modo = Modo.LIBRE;
                pianoView.unenablePausarGrabacionBoton();
                break;
            case "grabar":
                modo = Modo.GRABAR;
                cancion = new Cancion();
                pianoView.enablePausarGrabacionBoton();
                break;
            case "reproducir":
                ...
                for (Cancion c: canciones) {
                    if (pianoView.popUpSeleccionarCancion(nombreCanciones).equals(c.getNombre())){
                        this.cancion = c;
                        reproducirCancion();
                        break;
                    }
    ...
    private void reproducirCancion()  {
        ...
        for (FiguraMusical fm: figurasMusical) {
            ...
            TimerTask timerTaskpintarRosa = new TimerTask() {
                @Override
                public void run() {
                    pianoView.cambiarColorTecla(fm.getNota().toString() + String.valueOf(fm.getEscala()), Color.PINK);
                }
            };
            ...
            TimerTask timerTaskpintarNegro = new TimerTask() {
                ...
                public void run() {
                    if (fm.getNota().toString().charAt(fm.getNota().toString().length()-1) == '#') {
                        pianoView.cambiarColorTecla(fm.getNota().toString() + String.valueOf(fm.getEscala()), Color.black);
                    } else {
                        pianoView.cambiarColorTecla(fm.getNota().toString() + String.valueOf(fm.getEscala()), Color.white);
                    }
                }
            };
